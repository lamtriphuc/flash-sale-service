...
import com.baas.flashsale.media.CloudinaryImageService;
import com.baas.flashsale.media.CloudinaryUploadResult;
...
public class CampaignService {
    ...
    public List<CampaignResponse> getAllCampaigns() {
        Long tenantId = AuthenticatedUser.tenantId();
    ...
    public CampaignResponse getCampaign(Long campaignId) {
        return campaignMapper.toResponse(findCampaignForTenant(campaignId, AuthenticatedUser.tenantId()));
    }
    ...
    public FlashSaleItemResponse addItem(Long campaignId, CreateFlashSaleItemRequest request) {
        Campaign campaign = findCampaignForTenant(campaignId, AuthenticatedUser.tenantId());
    ...
    public ThumbnailUploadResponse uploadThumbnail(Long campaignId, MultipartFile file) {
        Campaign campaign = findCampaignForTenant(campaignId, AuthenticatedUser.tenantId());
        String oldPublicId = campaign.getThumbnailPublicId();

        CloudinaryUploadResult uploadResult = cloudinaryImageService.uploadCampaignThumbnail(campaignId, file);
        campaign.setThumbnailUrl(uploadResult.optimizedUrl());
        campaign.setThumbnailPublicId(uploadResult.publicId());
        campaignRepository.save(campaign);

        cloudinaryImageService.deleteQuietly(oldPublicId);
